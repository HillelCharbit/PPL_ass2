...
const applyClass = (proc: Class, args: Value[], env: Env): Result<Value> => {
    ...
    const methods_renamed_substituted = allT(isProcExp, methods) ? map((method) => makeProcExp(method.args, substitute(renameExps(method.body), fields, litArgs)), methods): methods;

    return makeOk(makeObject(zipWith(makeBinding, methodsNames, methods_renamed_substituted)));
};
